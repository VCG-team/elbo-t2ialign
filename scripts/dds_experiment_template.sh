...
# diffusion.variant options: runwayml/stable-diffusion-v1-5, CompVis/stable-diffusion-v1-4, stabilityai/sdxl-turbo, stabilityai/sd-turbo, stabilityai/stable-diffusion-2-1-base, stabilityai/stable-diffusion-xl-base-1.0, stabilityai/stable-diffusion-2
segmentation_args=$(cat << EOS
save_img=${save_img}
save_cross_att=${save_cross_att}
use_cls_predict=${use_cls_predict}
optimize_timesteps=[["random",50,949,25]]
...
do
    ...
    # 2. segmentation
    CUDA_VISIBLE_DEVICES=${device} python segmentation_with_dds.py --dataset-cfg ./configs/dataset/${dataset}.yaml output_path.${dataset}=${output_path}/${dataset}${dataset_suffix} data_name_list=./data/${dataset}/val_id${dataset_suffix}.txt ${segmentation_args}
    # 3. evaluation
